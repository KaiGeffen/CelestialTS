...
class TutorialRegion {
  ...
  create(): void {
    ...
    // Expansion icons
    let iconParamountcy = new Icon(this.scene, this.menu, -xDelta, yDelta, 'Paramountcy', function() {
      that.scene.scene.start("ParamountcyCatalogScene")
    })
  ...
  private createCheckMarks(xDelta: number, yDelta: number): void {
    ...
    // Expansion decks
    if (UserProgress.contains('tutorialCompleteParamountcy')) {
